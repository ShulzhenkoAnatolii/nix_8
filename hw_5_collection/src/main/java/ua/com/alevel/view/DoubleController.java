...
public class DoubleController {
    ...
    public void run() {
        ...
        String position;
        try {
            System.out.println("Start Program Double MathSet");
            menuProgram();
    ...
    private void add(BufferedReader reader) {
        System.out.print("Enter double numbers through a space, example ---> 2.5 4.7 -6.2 8.5 10.9\n" +
                "---> ");
        try {
            ...
            for (int i = 0; i < numbers.length; i++) {
                numbers[i] = Double.valueOf(array[i]);
            }
            mathSet.add(numbers);
    ...
    public void join(BufferedReader reader) {
        ...
        if (mathSet.size() != 0) {
            try {
                ...
                MathSet<Double> inputMathSet = new MathSet<>(numbers);
